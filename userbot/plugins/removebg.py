...
@borg.on(admin_cmd(pattern="rbg ?(.*)", outgoing=True))
@bot.on(sudo_cmd(pattern="rbbg ?(.*)", allow_sudo=True))
async def _(event):
    HELP_STR = (
        "`.rbg` as reply to a media, or give a link as an argument to this command"
    )
    if event.fwd_from:
        return
    ...
    if "image" in contentType:
        with io.BytesIO(output_file_name.content) as remove_bg_image:
            remove_bg_image.name = "Morag_RM_BG.png"
        ...
        await edit_or_reply(event, 
            "Removed dat annoying Backgroup in {} seconds, powered by © @moragmods ™".format(
                ms
            )
        )
    else:
        await edit_or_reply(event, 
            "ReMove.BG API returned Errors. Please report to @moragmods\n`{}".format(
...
CmdHelp("removebg").add_command(
  "rbg", "<reply to img>", "Removes that annoying background from the replied image. NEED TO GET A API KEY"
).add()
