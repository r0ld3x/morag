...
KANGING_STR = [
    ...
    "Mr.Morag Your Sticker is stealing this sticker... ",
]
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"HB_{user.username}_Ultron_{pack}"
        packnick = (
            f"{hellbot} Vol.{pack}"
            if hellbot
            else f"@{user.username}'s @moragmods Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
            ...
            async with bot.conversation("Stickers") as conv:
                ...
                while "120" in x.text:
                    ...
                    packnick = (
                        f"{hellbot} Vol.{pack}"
                        if hellbot
                        else f"@{user.username}'s Ultron Vol.{pack}"
                    )
                    await args.edit(
                        "`Switching to Pack "
                        + str(pack)
        ...
        await args.edit(
            f"** This Sticker Is [kanged](t.me/addstickers/{packname}) Successfully To Your Pack **⚡",
            parse_mode="md",
        )
