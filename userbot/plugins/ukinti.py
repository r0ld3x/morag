...
@bot.on(admin_cmd(pattern="ginfo ?(.*)"))
@bot.on(sudo_cmd(pattern="ginfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    if event.is_private:
...
CmdHelp("ukinti").add_command(
  "ginfo", None, "Gives the data of group. Deleted accounts, Last seen, Offline, Online, Recently, Bots, Etc."
).add_command(
